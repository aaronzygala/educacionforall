...
export function BlogCard({ blogPost }: { blogPost: Post }) {
  return (
    <Link href={"post/" + blogPost.uri}>
      <Card className="text-left rounded-md border-none shadow-lg hover:shadow-2xl hover:cursor-pointer transition-all delay-50 w-[100%]">
        ...
        <CardContent className="flex flex-col h-[250px] w-[400px]">
          <div className="text-lg tracking-tight font-bold py-4 text-nowrap text-ellipsis overflow-hidden">
            {blogPost.title}
          </div>
          <div
            className="text-sm h-[100px] text-ellipsis overflow-hidden"
